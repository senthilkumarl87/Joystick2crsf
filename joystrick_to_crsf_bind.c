...
struct BindingConfig {
    uint8_t binding_active;
    uint8_t bind_phase;
    uint32_t bind_start_time;
    uint64_t radio_id;
    uint8_t bind_payload[64];
    size_t bind_payload_len;
};
...
struct JoystickToCRSF {
    ...
    struct BindingConfig binding;
};

// Generate random radio ID (simulates hardware UUID)
uint64_t generate_radio_id() {
    uint64_t id = 0;
    ...
    if (fd >= 0) {
        ...
        id = (uint64_t)tv.tv_sec << 32 | tv.tv_usec;
    }
    return id & 0x0000FFFFFFFFFFFF; // 6-byte ID as per CRSF spec
}
...
// Generate Radio ID frame (for binding)
void generate_radio_id_frame(uint8_t* frame, size_t* frame_len, uint64_t radio_id) {
    int frame_index = 0;
    frame[frame_index++] = CRSF_SYNC_BYTE;     // Sync byte
    frame[frame_index++] = 8;                  // Frame size: type(1) + payload(6) + CRC(1) = 8
    frame[frame_index++] = CRSF_FRAMETYPE_RADIO_ID; // Frame type
    
    // Add 6-byte radio ID (little endian)
    for (int i = 0; i < 6; i++) {
        frame[frame_index++] = (radio_id >> (i * 8)) & 0xFF;
    }
...
// Generate Bind frame
void generate_bind_frame(uint8_t* frame, size_t* frame_len, uint64_t radio_id) {
    int frame_index = 0;
    frame[frame_index++] = CRSF_SYNC_BYTE;     // Sync byte
    frame[frame_index++] = 10;                 // Frame size: type(1) + payload(8) + CRC(1) = 10
    frame[frame_index++] = CRSF_FRAMETYPE_BIND; // Frame type
    
    // Bind payload structure
    frame[frame_index++] = 0x00; // Bind type (0 = standard bind)
    frame[frame_index++] = 0x00; // Reserved
...
// UDP functions
int udp_init(struct UDPConfig *udp, const char* dest_ip, int dest_port) {
    ...
    if (inet_pton(AF_INET, dest_ip, &udp->dest_addr.sin_addr) <= 0) {
        fprintf(stderr, "Invalid address/Address not supported: %s\n", dest_ip);
        close(udp->sockfd);
        return -1;
    }
    
    strncpy(udp->dest_ip, dest_ip, sizeof(udp->dest_ip) - 1);
...
int udp_send_frame(struct UDPConfig *udp, const uint8_t* frame, size_t frame_len) {
    if (!udp->enabled || udp->sockfd < 0) {
        return -1;
    }
    
    // Create packet with 4-byte header (big-endian frame length)
    uint8_t packet[frame_len + 4];
    uint32_t network_len = htonl(frame_len);
    memcpy(packet, &network_len, 4);
    memcpy(packet + 4, frame, frame_len);
    
    ssize_t sent = sendto(udp->sockfd, packet, frame_len + 4, 0,
                         (const struct sockaddr*)&udp->dest_addr, sizeof(udp->dest_addr));
...
// Binding functions
void binding_init(struct BindingConfig *binding) {
    binding->binding_active = 0;
    binding->bind_phase = 0;
    binding->bind_start_time = 0;
    binding->radio_id = generate_radio_id();
    memset(binding->bind_payload, 0, sizeof(binding->bind_payload));
    binding->bind_payload_len = 0;
    
    printf("🔐 Binding System Initialized\n");
    printf("   Radio ID: %012llX\n", (unsigned long long)binding->radio_id);
}

void start_binding(struct BindingConfig *binding) {
    binding->binding_active = 1;
    binding->bind_phase = 1;
    binding->bind_start_time = (uint32_t)time(NULL);
    printf("🚀 STARTING BINDING PROCESS\n");
    printf("   Radio ID: %012llX\n", (unsigned long long)binding->radio_id);
    printf("   Send bind command to receiver within 60 seconds...\n");
}

void update_binding(struct BindingConfig *binding) {
    if (!binding->binding_active) return;
    
    uint32_t current_time = (uint32_t)time(NULL);
    uint32_t elapsed = current_time - binding->bind_start_time;
    
    switch (binding->bind_phase) {
        case 1: // Send bind frames for 60 seconds
            if (elapsed < 60) {
                if (elapsed % 5 == 0) { // Print status every 5 seconds
                    printf("   🔄 Binding in progress... %d seconds remaining\n", 60 - elapsed);
                }
            } else {
                binding->bind_phase = 2;
                printf("   ✅ Bind frames sent. Waiting for receiver confirmation...\n");
            }
            break;
            
        case 2: // Wait for confirmation (simulated)
            if (elapsed > 70) {
                binding->binding_active = 0;
                printf("   ✅ Binding completed successfully!\n");
                printf("   📡 Receiver is now bound to Radio ID: %012llX\n", 
                       (unsigned long long)binding->radio_id);
            }
            break;
    }
...
void joystick_init(struct JoystickToCRSF *converter, const char* device, const char* udp_ip, int udp_port) {
    ...
    if (udp_ip && udp_port > 0) {
        if (udp_init(&converter->udp, udp_ip, udp_port) == 0) {
            printf("📡 UDP streaming enabled to %s:%d\n", udp_ip, udp_port);
        } else {
            fprintf(stderr, "⚠️  UDP initialization failed, continuing without UDP\n");
            converter->udp.enabled = 0;
        }
    } else {
        converter->udp.enabled = 0;
        printf("ℹ️  UDP streaming disabled (use --udp IP:PORT to enable)\n");
    }
    
    // Initialize binding system
    binding_init(&converter->binding);
...
void update_aux_channels(struct JoystickToCRSF *converter) {
    ...
    // Binding controls (Buttons 8 and 9)
    static int last_bind_button = 0;
    static int last_unbind_button = 0;
    
    if (converter->buttons[8] && !last_bind_button) {
        start_binding(&converter->binding);
    }
    last_bind_button = converter->buttons[8];
    
    if (converter->buttons[9] && !last_unbind_button) {
        converter->binding.binding_active = 0;
        printf("   🛑 Binding cancelled\n");
    }
    last_unbind_button = converter->buttons[9];
}
...
void print_status(struct JoystickToCRSF *converter, double current_jitter_ns) {
    ...
    // Binding status
    if (converter->binding.binding_active) {
        printf(" | 🔄 BINDING");
    } else {
        printf(" | 📡 ID: %012llX", (unsigned long long)converter->binding.radio_id);
    }
    ...
    // UDP statistics
    if (converter->udp.enabled) {
        printf(" | UDP: %d", converter->udp.packets_sent);
    }
...
void joystick_run(struct JoystickToCRSF *converter) {
    ...
    if (converter->udp.enabled) {
        printf("📡 UDP Streaming to %s:%d\n", converter->udp.dest_ip, converter->udp.dest_port);
    }
    printf("🔐 Binding Controls: Button 8=Start Bind, Button 9=Cancel Bind\n");
    printf("--------------------------------------------------\n");
    ...
    uint8_t bind_frame[64];
    size_t frame_len;
    size_t bind_frame_len;
    
    int bind_frame_counter = 0;
    
    while (running) {
        // Read all available joystick events (non-blocking)
        read_joystick_events(converter);
        
        // Update binding state machine
        update_binding(&converter->binding);
        ...
        // Send normal RC frame via UDP
        if (converter->udp.enabled) {
            udp_send_frame(&converter->udp, crsf_frame, frame_len);
        }
        
        // Send binding frames if active
        if (converter->binding.binding_active) {
            bind_frame_counter++;
            
            // Send bind frame every 10th frame (5Hz during binding)
            if (bind_frame_counter % 10 == 0) {
                generate_bind_frame(bind_frame, &bind_frame_len, converter->binding.radio_id);
                if (converter->udp.enabled) {
                    udp_send_frame(&converter->udp, bind_frame, bind_frame_len);
                }
                
                // Also send radio ID frame
                generate_radio_id_frame(bind_frame, &bind_frame_len, converter->binding.radio_id);
                if (converter->udp.enabled) {
                    udp_send_frame(&converter->udp, bind_frame, bind_frame_len);
                }
            }
        ...
        if (converter->frames_generated % 50 == 0) {
            ...
            if (converter->frames_generated == 0 || converter->frames_generated % 100 == 0) {
                ...
                // Also print bind frame if active
                if (converter->binding.binding_active) {
                    generate_bind_frame(bind_frame, &bind_frame_len, converter->binding.radio_id);
                    printf("BIND Frame: ");
                    print_crsf_frame(bind_frame, bind_frame_len);
                }
            }
    ...
    if (converter->udp.enabled) {
        printf("  UDP Packets Sent: %d\n", converter->udp.packets_sent);
    }
    printf("  Radio ID: %012llX\n", (unsigned long long)converter->binding.radio_id);
...
void print_usage(const char* program_name) {
    ...
    printf("Options:\n");
    printf("  --device DEVICE    Joystick device (default: /dev/input/js0)\n");
    printf("  --udp IP:PORT      Enable UDP streaming to specified IP:PORT\n");
    printf("  --bind             Start binding immediately\n");
    printf("  --help             Show this help message\n");
    printf("\nExamples:\n");
    printf("  %s --device /dev/input/js0\n", program_name);
    printf("  %s --udp 127.0.0.1:8888\n", program_name);
    printf("  %s --device /dev/input/js1 --udp 192.168.1.100:8888 --bind\n", program_name);
}
...
int main(int argc, char *argv[]) {
    ...
    int udp_port = 0;
    int start_binding_flag = 0;
    
    // Parse command line arguments
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--device") == 0 && i + 1 < argc) {
            joystick_device = argv[++i];
        } else if (strcmp(argv[i], "--udp") == 0 && i + 1 < argc) {
            char* udp_arg = argv[++i];
            char* colon = strchr(udp_arg, ':');
            if (colon) {
                *colon = '\0';
                udp_dest = udp_arg;
                udp_port = atoi(colon + 1);
            } else {
                fprintf(stderr, "Invalid UDP format. Use IP:PORT\n");
                return 1;
            }
        } else if (strcmp(argv[i], "--bind") == 0) {
            start_binding_flag = 1;
    ...
    joystick_init(&converter, joystick_device, udp_dest, udp_port);
    
    // Start binding if requested
    if (start_binding_flag) {
        start_binding(&converter.binding);
    }
