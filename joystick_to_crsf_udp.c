...
int udp_send_raw_frame(struct UDPConfig *udp, const uint8_t* frame, size_t frame_len) {
    if (!udp->enabled || udp->sockfd < 0) {
        return -1;
    }
    
    // Send raw frame without header (for compatibility)
    ssize_t sent = sendto(udp->sockfd, frame, frame_len, 0,
                         (const struct sockaddr*)&udp->dest_addr, sizeof(udp->dest_addr));
...
void joystick_run(struct JoystickToCRSF *converter) {
    ...
    while (running) {
        ...
        if (converter->udp.enabled) {
            ...
            udp_send_frame(&converter->udp, crsf_frame, frame_len);  // With header
            // udp_send_raw_frame(&converter->udp, crsf_frame, frame_len); // Without header
        }
...
void print_usage(const char* program_name) {
    ...
    printf("  %s --device /dev/input/js1 --udp 192.168.1.100:8888\n", program_name);
}
